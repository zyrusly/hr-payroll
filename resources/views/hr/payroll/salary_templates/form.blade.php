                ...
                <div class="content_wrapper content-padded">
                    <form method="POST" action="{{ $mode === 'edit' ? route('payroll.salary-templates.update', $template) : route('payroll.salary-templates.store') }}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 form-group mb-3"><label>{{ __('Pay Frequency') }}</label><select name="pay_frequency" class="form-control" required>@foreach(['monthly','weekly','semimonthly'] as $frequency)<option value="{{ $frequency }}" {{ old('pay_frequency', $template->pay_frequency ?? 'monthly')===$frequency?'selected':'' }}>{{ __(ucfirst($frequency)) }}</option>@endforeach</select></div>
